...
import { Box, CssBaseline, type SelectChangeEvent } from "@mui/material";
...
function App() {
  ...
  const handleApplyItf = useCallback(
    (content: string) => {
      applyItf(content);
    },
    [applyItf],
  ...
  useEffect(() => {
    ...
    if (path.startsWith("/code")) {
      const filePath = path.startsWith("/code/")
        ? path.substring("/code/".length)
        : "";
      ...
      if (filePath) {
        ...
        // No file path in URL, e.g., /code or /code/
        clearActiveFile();
        if (sourceTree) {
    ...
    <Box sx={{ display: "flex", height: "100vh" }}>
      ...
      >
        <TopBar
          ...
          view={view}
          title={
            view === "code" ? "Code" : view === "git" ? "Git" : displayedTitle
          }
        ...
        <Routes>
          ...
          <Route
            path="/git/*"
            element={<GitBrowser log={gitGraphLog} commitDiff={commitDiff} />}
          />
          <Route
            path="/*"
            element={
